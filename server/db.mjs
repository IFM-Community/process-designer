...
export function upsertUser({ email, name = '', avatar = '' }) {
...
export const getUser = (id) => q.userById.get(id) || null
...
export function createStudio(name, ownerUserId) {
  const id = uid('st')
...
export const listStudios = (userId) =>
  q.studiosForUser.all(userId).map((s) => ({ ...s, processes: q.countProcesses.get(s.id).c }))

export const membership = (studioId, userId) => q.member.get(studioId, userId) || null
export const isMember = (studioId, userId) => !!q.member.get(studioId, userId)
export const listMembers = (studioId) => q.membersOf.all(studioId)
export function renameStudio(studioId, name) { q.renameStudio.run(name, studioId); return { ok: true } }

// ---- Invites --------------------------------------------------------------

export function createInvite(studioId, email, role, invitedBy) {
  const e = String(email).trim().toLowerCase()
...
export const listInvites = (studioId) => q.invitesForStudio.all(studioId)
...
export const endAuthSession = (token) => { if (token) q.delAuth.run(token) }

// ---- Studio-scoped state --------------------------------------------------

export function readState(studioId) {
...
export function writeState(studioId, { sessions = [], activeId, geom }) {
  const ts = now()
...
export function deleteSession(studioId, id) {
  q.deleteSession.run(id, studioId)
  return { ok: true, id }
}

// Only serve revisions for a process that lives in the caller's studio.
export const sessionStudioId = (id) => q.sessionStudio.get(id)?.studio_id || null
export const listRevisions = (sessionId) => q.listRevisions.all(sessionId)
...
export function migrateOrphans() {
