...
import {
  DB_FILE, readState, writeState, deleteSession, listRevisions, getRevision,
...
const mePayload = (u) => ({ user: publicUser(u), studios: listStudios(u.id) })
...
export async function handleApi(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && pathname === '/api/auth/mock-login') {
      ...
      const user = upsertUser({ email, name, avatar })
      const token = createAuthSession(user.id)
      return json(res, 200, mePayload(user), { 'Set-Cookie': setAuthCookie(token) })
    }
    if (req.method === 'POST' && pathname === '/api/auth/logout') {
      endAuthSession(parseCookies(req)[AUTH_COOKIE])
      return json(res, 200, { ok: true }, { 'Set-Cookie': clearAuthCookie() })
    }
    if (req.method === 'GET' && pathname === '/api/me') {
      const u = userOf(req)
      return u ? json(res, 200, mePayload(u)) : json(res, 401, { error: 'not signed in' })
    }
    ...
    const user = userOf(req) || (REQUIRE_AUTH ? null : ensureDefaultUser())
    if (!user) return json(res, 401, { error: 'not signed in' })

    // ---- Studios -------------------------------------------------------
    if (req.method === 'GET' && pathname === '/api/studios') {
      return json(res, 200, { studios: listStudios(user.id) })
    }
    if (req.method === 'POST' && pathname === '/api/studios') {
      const { name } = await readBody(req)
      const studio = createStudio((name || '').trim() || 'New studio', user.id)
      return json(res, 200, { studio, studios: listStudios(user.id) })
    }
    ...
    if (sm) {
      ...
      const sub = sm[2]
      const mem = membership(studioId, user.id)
      if (!mem) return json(res, 403, { error: 'not a member of this studio' })

      if (sub === 'members' && req.method === 'GET') return json(res, 200, { members: listMembers(studioId) })
      if (sub === 'invites' && req.method === 'GET') return json(res, 200, { invites: listInvites(studioId) })
      if (sub === 'invites' && req.method === 'POST') {
        ...
        if (!email || !String(email).includes('@')) return json(res, 400, { error: 'a valid email is required' })
        const r = createInvite(studioId, email, role === 'viewer' ? 'viewer' : 'editor', user.email)
        return json(res, 200, { ...r, members: listMembers(studioId), invites: listInvites(studioId) })
      }
      if (sub === 'rename' && req.method === 'POST') {
        if (mem.role !== 'owner') return json(res, 403, { error: 'only the owner can rename the studio' })
        const { name } = await readBody(req)
        renameStudio(studioId, (name || '').trim() || 'Studio')
        return json(res, 200, { ok: true, studios: listStudios(user.id) })
      }
    }
    ...
    const studioId = url.searchParams.get('studio') || listStudios(user.id)[0]?.id
    const requireMember = () => studioId && isMember(studioId, user.id)

    if (req.method === 'GET' && pathname === '/api/state') {
      if (!requireMember()) return json(res, 403, { error: 'not a member of this studio' })
      return json(res, 200, readState(studioId))
    }
    if (req.method === 'PUT' && pathname === '/api/state') {
      ...
      return json(res, 200, writeState(studioId, body))
    }
    if (req.method === 'DELETE' && pathname === '/api/session') {
      if (!requireMember()) return json(res, 403, { error: 'not a member of this studio' })
      const id = url.searchParams.get('id')
      if (!id) return json(res, 400, { error: 'id required' })
      return json(res, 200, deleteSession(studioId, id))
    }
    ...
    if (req.method === 'GET' && pathname === '/api/revisions') {
      ...
      const st = sessionStudioId(id)
      if (!st || !isMember(st, user.id)) return json(res, 403, { error: 'no access' })
      return json(res, 200, { revisions: listRevisions(id) })
    }
    if (req.method === 'GET' && pathname === '/api/revision') {
      const rev = getRevision(Number(url.searchParams.get('id')))
      if (!rev) return json(res, 404, { error: 'not found' })
      const st = sessionStudioId(rev.id)
      if (!st || !isMember(st, user.id)) return json(res, 403, { error: 'no access' })
      return json(res, 200, rev)
    }

    if (req.method === 'GET' && pathname === '/api/health') {
      return json(res, 200, { ok: true, db: DB_FILE })
    }
    return false
  } catch (e) {
